...
function searchVideos() {
  let searchKeyword = document.getElementById("txt-search").value.toLowerCase();
  if (searchKeyword === "") {     // the search button is pressed with no keyword
    alert("Please enter a keyword");
  } else {
    let searchResults = [];       // array holding the videos to be displayed
    document.getElementById("txt-search").value = "";   // clear the input textbox
    videosList.forEach((video) => {     // loop on the videos list to search it
      if (video.description.toLowerCase().search(searchKeyword) !== -1) {   // found a video
        searchResults.push(video);    // push it in the results array to be displayed
      }
    });
    if (searchResults.length > 0) {   // there are videos to be displayed
      document.getElementById("results").innerHTML = "";
      searchResults.forEach((video) => {    // loop on the results and add them in #results div
        document.getElementById("results").innerHTML += generateRowHTML(video);
      });
    } else {            // there are no videos to be displayed
      document.getElementById(
        "results"
      ).innerHTML = `No videos found with keyword ${searchKeyword}`;
    }
  }
}

// show all videos function
function showAllVideos() {
  document.getElementById("results").innerHTML = "";
  videosList.forEach((video) => {     // loop on videos list and display every video
    document.getElementById("results").innerHTML += generateRowHTML(video);
  });
}

document.getElementById("btn-search").onclick = searchVideos;
document.getElementById("btn-show-all").onclick = showAllVideos;
